...
namespace Demot.RandomOrgJsonRPC
{
    ...
    /// </summary>
    public class RandomJsonRPCClient
    {
        const string
            // HTTP header
        ...
        public RandomJsonRPCClient(string apiKey) : this(apiKey, null) { }
        /// <summary>
        /// Intializes a new instance of the RandomOrgApiClient.
        /// </summary>
        /// <param name="apiKey">API key, used to keep track of the true random bit usage.</param>
        ...
        /// <exception cref="System.ArgumentNullException"></exception>
        public RandomJsonRPCClient(string apiKey, string userAgent) {
        ...
        /// <param name="signed">Specifies if the result should be signed with a SHA-512 key which you can use to verify
        ///                      it against random.org's public key.</param>
        ...
        /// <param name="replacement">Specifies if numbers should be picked with replacement. If true, the generated
        ///                           numbers may contain duplicates, otherwise the picked numbers are unique.</param>
        /// <exception cref="System.ArgumentOutOfRangeException"></exception>
        /// <exception cref="System.TimeoutException"></exception>
        /// <exception cref="ProtocolViolationException"></exception>
        public Response GenerateStrings (
            int n,
            int length,
            int id,
            bool signed,
            string characters = StringCharacters,
            bool replacement = DefaultReplacement)
        {
            if (n < 1 || n > maxN)
                throw new ArgumentOutOfRangeException (
                    String.Format (ArgumentRangeException, "n", 1, maxN));
            if (!String.IsNullOrEmpty (characters) && characters.Length > 80)
                throw new ArgumentOutOfRangeException (
                    String.Format (ArgumentRangeException, "characters length", 1, 80));
            else
        ...
        /// <exception cref="System.TimeoutException"></exception>
        /// <exception cref="ProtocolViolationException"></exception>
        public Response GenerateStrings (int n,
            int length,
            int id,
            string characters = StringCharacters,
            bool replacement = DefaultReplacement)
        ...
        /// <exception cref="System.TimeoutException"></exception>
        /// <exception cref="ProtocolViolationException"></exception>
        public Response GenerateIntegers (
            int n,
            int min,
            int max,
            int id,
            ...
            bool replacement = DefaultReplacement)
        {
            ...
            if (min < -rangeMinMax || min > rangeMinMax || max < -rangeMinMax || max > rangeMinMax)
                throw new ArgumentOutOfRangeException (
                    String.Format (ArgumentRangeException, "Range", -rangeMinMax, rangeMinMax));
        ...
        /// <exception cref="System.TimeoutException"></exception>
        /// <exception cref="ProtocolViolationException"></exception>
        public Response GenerateDecimalFractions (
            int n,
            byte decimalPlaces,
            int id,
            bool signed,
            bool replacement = DefaultReplacement)
        {
            ...
            if (decimalPlaces < 1 || decimalPlaces > 20)
                throw new ArgumentOutOfRangeException (
                    String.Format (ArgumentRangeException, "decimalPlaces", 1, 20));
        ...
        /// <param name="standardDeviation">Distribution's standard deviation, must be within the [-1e6, 1e6]
        ///                                 range.</param>
        /// <param name="significantDigits">Number of significant digits to use, must be within the [2, 20]
        ///                                 range.</param>
        /// <param name="id">User defined number which will be returned in the response.</param>
        /// <param name="signed">Specifies if the result should be signed with a SHA-512 key which you can use to verify
        ///                      it against random.org's public key.</param>
        ...
        /// <exception cref="System.TimeoutException"></exception>
        /// <exception cref="ProtocolViolationException"></exception>
        public Response GenerateGaussians (
            int n,
            double mean,
            ...
            bool signed)
        {
            ...
            if (mean < -1e6 || mean > 1e6)
                throw new ArgumentOutOfRangeException (
                    String.Format (ArgumentRangeException, "mean", -1e6, 1e6));
            if (standardDeviation < -1e6 || standardDeviation > 1e6)
                throw new ArgumentOutOfRangeException (
                    String.Format (ArgumentRangeException, "standardDeviation", -1e6, 1e6));
            if (significantDigits < 2 || significantDigits > 20)
                throw new ArgumentOutOfRangeException (
                    String.Format (ArgumentRangeException, "significantDigits", 2, 20));
            ...
            bool signed)
        {
            if (n < 1 || n > 1000)
                throw new ArgumentOutOfRangeException (
                    String.Format (ArgumentRangeException, "n", 1, 1000));
            var parameters = JsonHelper.GetString (false,
                ParameterApiKey, apiKey,
                ParameterCount, n);
            ...
            string format = "base64")
        {
            if (n < 1 || n > 100)
                throw new ArgumentOutOfRangeException (
                    String.Format (ArgumentRangeException, "n", 1, 100));
            if (size < 1 || size > 1024 * 1024)
                throw new ArgumentOutOfRangeException (
                    String.Format (ArgumentRangeException, "size", 1, 1024 * 1024));
            if (size % 8 != 0)
